...
- [RAG 정량평가 - 개선 전](docs/evaluation/월드비전_AI_최종_평가_개선_전.xlsx)
- [RAG 정량평가 - 개선 후](docs/evaluation/월드비전_AI_최종_평가_개선_후.xlsx)
...
최종 배포된 서비스를 대상으로 PO 관점에서 직접 사용성 검수를 진행하였습니다.

### 좋았던 점
- 블루 & 화이트 컬러 조합으로 구성되어 있어 화면의 가독성이 좋고 눈의 피로감이 적었습니다.
- 지식검색, 회의록 및 보고서 생성 등 전반적인 기능의 응답 속도가 빠른 편이었습니다.

### 개선 및 불편사항
- 시작 화면에 제공되는 예시 질문 중 일부(예: `아동 후원 업무 절차를 정리해줘`)는 실제 사내 문서에 관련 정보가 충분하지 않아 정확한 답변을 얻기 어려운 경우가 있었습니다.
- 사용자가 예시 질문을 서비스가 지원하는 대표 질문으로 인식할 수 있으므로, 실제 보유 문서를 기반으로 안정적인 답변이 가능한 질문 위주로 예시를 구성할 필요가 있습니다.

### 향후 개선 방향
- 실제 검색 가능한 사내 문서의 범위를 기준으로 예시 질문을 재구성하여 초기 사용자 경험을 개선할 예정입니다.
- 향후 내부 문서 데이터가 확대될 경우 지원 가능한 질문 범위도 함께 확장할 수 있습니다.
